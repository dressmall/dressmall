			          				...
			          				<c:forEach var="p" items="${paymentList }">
			          					<div class="payment-box d-flex align-items-center justify-content-between mb-3">
						          			...
						          			<div>
							          			...
							          			<!-- 결제완료일 경우 -->
							          			<c:if test="${p.paymentState == '결제완료' }">
							          				<a
							          					href="${pageContext.request.contextPath}/on/customer/removeOrders?paymentNo=${p.paymentNo}"
							          					 class="btn-main ms-3">
							          					주문취소	
							          				</a>
							          			</c:if>		
							          			<!-- 배송중일 경우 -->	          			
							          			<c:if test="${p.paymentState == '배송중' }">
							          				<a	href="${pageContext.request.contextPath}/on/customer/modifyOrders?paymentNo=${p.paymentNo }"
							          					class="btn-main ms-3">구매확정</a>
							          			</c:if>			 
							          			<!-- 배송완료일 경우 -->         			
							          			<c:if test="${p.paymentState == '배송완료' }">
